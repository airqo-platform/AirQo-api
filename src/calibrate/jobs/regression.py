...
# channel 88-thingspeak,  device colocated with MUK BAM
muk_lowcost_data = pd.read_csv("jobs/AQ_88.csv")
...
def process_data_lowcost(muk_lowcost_data):
    ...
    muk_lowcost_data = muk_lowcost_data.drop(
        ['entry_id', 'Latitude', 'Longitude', 'Battery Voltage', 'GpsData', 'latitude', 'longitude', 'elevation', 'status'], axis=1)
    muk_lowcost_data['Average_PM2.5_CF_1_ug/m3'] = muk_lowcost_data[[
        'Sensor1 PM2.5_CF_1_ug/m3', 'Sensor2PM2.5_CF_1_ug/m3']].mean(axis=1).round(2)
    muk_lowcost_data['Average_PM10_CF_1_ug/m3'] = muk_lowcost_data[[
        'Sensor1 PM10_CF_1_ug/m3', 'Sensor2 PM10_CF_1_ug/m3']].mean(axis=1).round(2)

    muk_lowcost_data["TimeStamp"] = pd.to_datetime(
        muk_lowcost_data["created_at"])
    muk_lowcost_data.drop_duplicates(
...
def process_data_bam(muk_bam_data):
    muk_bam_data = muk_bam_data.drop(
        ['Flow(lpm)', 'WS(m/s)', 'WD(Deg)', 'BP(mmHg)', 'FT(C)', 'FRH(%)', 'Status'], axis=1)
    muk_bam_data["TimeStamp"] = pd.to_datetime(muk_bam_data["Time"])
    muk_bam_data.drop_duplicates(
        subset="TimeStamp", keep='first', inplace=True)
...
def combine_datasets(muk_lowcost_hourly_mean, muk_bam_data):
    ...
    muk_lowcost_hourly_mean["Time"] = pd.to_datetime(
        muk_lowcost_hourly_mean["Time"])
    ...
    hourly_combined_dataset = pd.merge(
        muk_lowcost_hourly_mean, hourly_BAM_data, on='Time')
    hourly_combined_dataset.rename(columns={
                                   'Average_PM2.5_CF_1_ug/m3': 'muk_lowcost_hourly_PM', 'ConcHR(ug/m3)': 'muk_bam_hourly_PM'}, inplace=True)

    hourly_combined_dataset['muk_bam_hourly_PM'] = hourly_combined_dataset['muk_bam_hourly_PM'].shift(
        -1)

    return hourly_combined_dataset


hourly_combined_dataset = combine_datasets(
    muk_lowcost_hourly_mean, muk_bam_data)
...
def Uncertainity_std(popt, pcov):
    ...
    return 'R^2: ' + str(r2), 'Uncertainty-Slope: ' + str(a), 'Uncertainty-Intercept: ' + str(b)
