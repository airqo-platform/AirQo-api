...
SignalsSchema.pre("save", function(next) {
  ...
  if (this.deployment_type === "static") {
    if (!this.site_id) {
      return next(new Error("Static signals require site_id"));
    }
    ...
    if (
      ...
      return next(
        new Error(
          "Mobile signals require either grid_id or location coordinates"
        )
      );
    }
...
SignalsSchema.statics.latest = async function(
  ...
  try {
    // Strategy: Use $lookup with pipeline to get only the latest signal per site
    ...
    const pipeline = [
      ...
      // 2. For each site, lookup only the latest signal
      {
        $lookup: {
          from: "signals", // Your collection name
          let: { siteId: "$_id" },
          pipeline: [
            {
              $match: {
            ...
            { $sort: { time: -1 } },
            { $limit: 1 }, // Only get the latest signal per site
          ],
          as: "latestSignal",
        },
      },
    ...
    if (!isEmpty(data)) {
      return {
        success: true,
        message: "Successfully retrieved latest signals",
        data,
        status: httpStatus.OK,
      };
    } else {
...
SignalsSchema.statics.latestForMap = async function(
  { filter = {}, limit = 1000, skip = 0 } = {},
  next
) {
  try {
    ...
    const pipeline = [
      ...
      // 3. Group by site_id and take first (latest) signal
      {
        $group: {
          _id: "$site_id",
      ...
      // 4. Replace root with the latest signal
      { $replaceRoot: { newRoot: "$latestSignal" } },
    ...
    return {
      success: true,
      message: "Successfully retrieved latest map signals",
      data,
      status: httpStatus.OK,
    };
  } catch (error) {
...
SignalsSchema.statics.recent = async function(
  { filter = {}, limit = 1000, skip = 0 } = {},
  next
) {
  try {
...
SignalsSchema.statics.getBestAirQualityLocations = async function(
...
SignalsSchema.statics.getAirQualityAnalytics = async function(siteId, next) {
  try {
    // Validate input
    if (!siteId) {
      return {
...
SignalsSchema.statics.getWorstPm2_5Signal = async function({
  siteIds = [],
  next,
} = {}) {
  try {
    ...
    if (!isEmpty(worstSignal)) {
      ...
      return {
        success: true,
        message:
          "No pm2_5 signals found for the specified site_ids in the last three days.",
        data: {},
        status: httpStatus.OK,
      };
    }
...
SignalsSchema.statics.listRecent = async function(
  { filter = {}, limit = 1000, skip = 0, page = 1 } = {},
  next
) {
  try {
    logText("Using optimized Signals collection for recent data query");

    // Build aggregation pipeline for Signals (much simpler than Events)
    const pipeline = [
      { $match: filter },
...
SignalsSchema.statics.viewRecent = async function(filter, next) {
  try {
    logText("Using optimized Signals collection for recent view query");
...
SignalsSchema.statics.listRecentOptimized = async function(
  { filter = {}, limit = 1000, skip = 0, page = 1 } = {},
  next
) {
  try {
    logText(
      "Using ultra-optimized Signals collection - no aggregations needed!"
    );
...
SignalsSchema.statics.getLatestByLocation = async function(
  ...
  try {
    ...
    return {
      success: true,
      message: "Successfully retrieved latest signals by location",
