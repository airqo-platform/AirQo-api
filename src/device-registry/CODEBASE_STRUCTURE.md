...
Adding a new environment variable requires editing the three tracked template files (`.env.{env}.template.json`) and the local development runtime file (`.env.development.json`). The staging and production runtime files (`.env.staging.json`, `.env.production.json`) are generated by the CI/CD pipeline from Azure Key Vault and must not be edited manually to avoid accidental secret handling. No JS file changes are needed. The JSON templates are the canonical reference for key names and expected shape, but they do not document non-obvious runtime behaviour — if a variable has special semantics, fallback behaviour, or invariants enforced in code (e.g. leaving `NOMINATIM_BASE_URL` blank uses the public OpenStreetMap endpoint; `PROTECTED_COHORT_NAMES` always includes `airqo` regardless of the configured value), document those here instead.
