...
const { TOPICS, getTopics, getTopic } = require("../utils/kafka-topics")
const manageSite = require("../utils/create-site")
...
const kafkaConsumer = async () => {
  ...
  logObject("Kafka Consumer Topics", topics)
  ...
  await consumer.run({
    eachMessage: async ({ topic, partition, message }) => {
      ...
      else if (topic == getTopic(TOPICS.SITE_DISTANCES)){
        try{
          const decodedValue = await schemaRegistry.decode(message.value);
          let site_distances = JSON.parse(JSON.stringify(decodedValue.sites));
  
          if (Array.isArray(site_distances)) {

            const tenant = site_distances.pop("tenant");
            const filter = {
              "latitude": site_distances.pop("latitude"),
              "longitude": site_distances.pop("longitude")
            }
            
            const response = manageSite.update(tenant, filter, site_distances);
            logObject("Update Response", JSON.stringify(response));
          }
          else{

            logObject("Invalid Array of site distances", site_distances );
          }
  
        }
        catch (e) {
          logObject("Kafka get site details consumer", e);
        }
      }

      else{
        throw new Error("Unexpected Error --- Invaid kafka topic : " + topic );
      }  
    },
  });
}
...
module.exports = { kafkaConsumer, rawEventsConsumerV2 };
