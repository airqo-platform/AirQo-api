...
# 'Placeholder1':'AQKNSLBV', 'Placeholder3':'ANYRC92F',
...
APP_CONFIG = app_config.get(os.getenv("FLASK_ENV"))


def connect_mongo(tenant):
    client = MongoClient(APP_CONFIG.MONGO_URI)
    db = client[f'{APP_CONFIG.DB_NAME}_{tenant.lower()}']
    return db
...
def get_device_code(organisation, location, tenant):
    '''
...
def get_last_time_from_device_hourly_measurements(device_code, tenant):
    ...
    last_record = list(db.device_hourly_measurements.find(
        query).sort([('time', -1)]).limit(1))
    last_time = last_record[0]['time']
    return last_time


def get_last_time_from_device_daily_measurements(device_code, tenant):
    ...
    db = connect_mongo(tenant)
    query = {'deviceCode': device_code}
    last_record = list(db.device_daily_measurements.find(
        query).sort([('time', -1)]).limit(1))
    last_time = last_record[0]['time']
    return last_time


def update_hourly_measurements(data, tenant):
    """
    ...
    for i in data:
        key = {'_id': i['_id']}
        db.device_hourly_measurements.update(key, i, upsert=True)


def update_daily_measurements(data, tenant):
    """
    ...
    for i in data:
        key = {'_id': i['_id']}
        db.device_daily_measurements.update(key, i, upsert=True)


def save_hourly_measurements(data, tenant):
    """
    Saves hourly measurements.
    """
    db = connect_mongo(tenant)
    for i in data:
        db.device_hourly_measurements.insert(i)


def save_daily_measurements(data, tenant):
    """
     Inserts daily measurements into device_daily_measurements collection.
    """
    db = connect_mongo(tenant)
    for i in data:
        db.device_daily_measurements.insert(i)


def insert_data_mongo(data, tenant):
    """
...
def get_last_time(device_code, tenant):
    """
    ...
    query = {'deviceCode': device_code}
    last_record = list(db.device_raw_measurements.find(
        query).sort([('time', -1)]).limit(1))
    last_time = last_record[0]['time']
    return last_time


def get_filtered_data(device_code, start_date=None, end_date=None, frequency='daily', pollutant='PM 2.5', tenant=""):
    """
...
def get_piechart_data(device_code, start_date=None, end_date=None, frequency='daily', pollutant='PM 2.5', tenant=""):
    '''
    ...
    if records:
        if pollutant == 'PM 2.5':
            #records = get_filtered_data(device_code, start_date, end_date, frequency, pollutant,tenant)
        ...
        tasks = [good_sum, moderate_sum, UH4SG_sum, unhealthy_sum,
                 v_unhealthy_sum, hazardous_sum, unknowns_sum]
...
def get_exceedances(pollutant='PM 2.5', standard='WHO', frequency='daily', tenant=""):
    '''
    ...
    for location in code_locations_dict.keys():
        exceedances_dict = {}
        if standard == 'AQI':
            exceedances = get_AQI_exceedances(
                location, pollutant, frequency, tenant)
            ...
            exceedances = get_WHO_exceedances(
                location, pollutant, frequency, tenant)
            exceedances_dict['location'] = location
...
def get_WHO_exceedances(location, pollutant='PM 2.5', frequency='daily', tenant=""):
    '''
    ...
    records = get_filtered_data(device_code, helpers.date_to_str(
        start_date), helpers.date_to_str(end_date), frequency, pollutant, tenant)
...
def get_AQI_exceedances(location, pollutant='PM 2.5', frequency='daily', tenant=""):
    '''
