...
class DailyAveragesResource(Resource):
    ...
    def post(self):
        tenant = request.args.get("tenant", "airqo")
        try:
            json_data = DailyAveragesSchema().load(dashboard_api.payload)
...
class DeviceDailyAveragesResource(Resource):
    @swag_from("/api/docs/dashboard/device_daily_measurements_get.yml")
    def post(self):
        tenant = request.args.get("tenant", "airqo")
        try:
            json_data = DeviceDailyAveragesSchema().load(dashboard_api.payload)
        except ValidationError as err:
            return (
                create_response(f" {err.messages}", success=False),
                Status.HTTP_400_BAD_REQUEST
...
class ExceedancesResource(Resource):
    ...
    def post(self):
        tenant = request.args.get("tenant", "airqo")
        try:
            json_data = ExceedancesSchema().load(dashboard_api.payload)
        except ValidationError as err:
            return (
                create_response(f" {err.messages}", success=False),
                Status.HTTP_400_BAD_REQUEST
...
class DeviceExceedancesResource(Resource):
    @swag_from("/api/docs/dashboard/exceedances_post.yml")
    def post(self):
        tenant = request.args.get("tenant", "airqo")
