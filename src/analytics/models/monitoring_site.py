...
class MonitoringSite():
    ...
    def get_location_devices_code(self, organisation_name, location_name):
        ...
        devices = []
        query = {"$and": [{"Organisation": organisation_name}, {
            "Parish": location_name}]}
        results = list(db.monitoring_site.find(query,
...
​
    ...
    def get_device_past_28_days_measurements(self, device_code):
        ...
        results = mongo_helpers.get_filtered_data(
            device_code, starttime, endtime, 'daily', 'PM 2.5', self.tenant)
    ...
    def get_all_devices_past_28_days_measurements(self):
        ...
        results = list(
            db.device_daily_historical_averages.aggregate([query, projection]))
        return results
