...
class Report():
    ...
    def get_default_report_template(self):
        db = mongo_helpers.connect_mongo(self.tenant)
        documents = db.report_template.find({"report_type": "default"})
        return documents
    ...
    def get_monthly_report(self, user_id):
        db = mongo_helpers.connect_mongo(self.tenant)
        documents = db.report_template.find({"user_id": str(user_id)})
        return documents

    # update previously saved report
    def update_default_report_template(self, report_body):
        db = mongo_helpers.connect_mongo(self.tenant)
        response = db.report_template.update_one(
    ...
    def delete_monthly_report(self, report_name):
        db = mongo_helpers.connect_mongo(self.tenant)
        response = db.report_template.delete_one({'report_name': report_name})
        return response
