...
import requests
import os
from convert_dates import str_to_date, date_to_str, str_to_date_find
...
def update_device_daily_measurements_core(device_code, average):
    ...
    the "average" could be raw. hourly or daily
    '''
    last_time = date_to_str(
        get_last_time_from_device_daily_measurements(device_code))
    ...
    api_url = CLARITY_API_BASE_URL + "/measurements?startTime="+last_time + \
        '&endTime='+endtime+"&code="+device_code+"&average="+average
    ...
    if len(json_results) == 0:
        ...
        while len(json_results) != 0:
            ...
            api_url = base_url+'startTime='+date_to_str(last_time) + '&endTime='+date_to_str(
                next_endtime)+'&code='+device_code + "&average="+average
            results = requests.get(api_url, headers=headers)
            json_results = results.json()
            if len(json_results) == 0:
                pass
