    ...
    def consume(self):
        ...
        for msg in consumer:
            try:
                ...
                bearing_from_kampala = self.__model.get_bearing_from_kampala(
                    latitude, longitude
                )
                distance_from_kampala = self.__model.get_distance_from_kampala(
                    latitude, longitude
                )
                distance_to_closest_road = self.__model.get_distance_to_closest_road(
                    latitude, longitude
                )
                distance_to_closest_primary_road = (
                    self.__model.get_distance_to_closest_primary_road(
                        latitude, longitude
                    )
                )
                distance_to_closest_secondary_road = (
                    self.__model.get_distance_to_closest_secondary_road(
                        latitude, longitude
                    )
                )
                distance_to_closest_residential_road = (
                    self.__model.get_distance_to_closest_residential_road(
                        latitude, longitude
                    )
                )
                distance_to_closest_tertiary_road = (
                    self.__model.get_distance_to_closest_tertiary_road(
                        latitude, longitude
                    )
                )
                distance_to_closest_trunk = self.__model.get_distance_to_closest_trunk(
                    latitude, longitude
                )
                distance_to_closest_unclassified_road = (
                    self.__model.get_distance_to_closest_unclassified_road(
                        latitude, longitude
                    )
                )
                distance_to_closest_motorway = (
                    self.__model.get_distance_to_closest_motorway(latitude, longitude)
                )
