...
# There is a need for logic that corresponds to particular type of DB.
...
class daoBaseClass:
    ...
    def __init__(self, databasepath, SQL_query=None, db_type, logf=None, _trace=0, _debug=0):
        # ----------------------------------------------------------------------
    ...
    def __setitem__(self, key, value):
        ...
        try:
            ...
            emsg = "Unable to set Field name '%s' to value=%s" % (
                key, str(value))
            if self.logf:
                self.logf.writelines("E", emsg)
            else:
