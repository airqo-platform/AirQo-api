...
from app.schemas.firmware import FirmwareRead, FirmwareUpdate
from app.models.firmware import FirmwareType
...
async def upload_firmware(
    ...
    try:
        firmware = crud_firmware.upload_firmware(
            db=db,
...
def get_firmware(
    ...
    firmware = crud_firmware.get(db=db, id=firmware_uuid)
    if not firmware:
...
def download_firmware_file_by_id(
    ...
    file_data, file_size, blob_path, _, _ = crud_firmware.download_firmware_file(
        db=db,
    ...
    if range_result:
        ...
        # Download with range
        file_data, file_size, blob_path, range_start, range_end = crud_firmware.download_firmware_file(
            db=db,
            firmware_id=firmware_uuid,
            file_type=file_type,
            bucket_name=settings.GCS_BUCKET_NAME,
...
def download_firmware_file(
    ...
    if firmware_id:
        ...
        firmware = crud_firmware.get_by_version(db=db, firmware_version=firmware_version)
        if not firmware:
            raise HTTPException(
                status_code=404,
        ...
        file_data, file_size, blob_path, _, _ = crud_firmware.download_firmware_file_by_version(
            db=db,
            firmware_version=firmware_version,
            file_type=file_type,
            bucket_name=settings.GCS_BUCKET_NAME
        ...
        if range_result:
            range_start, range_end = range_result
            file_data, file_size, blob_path, range_start, range_end = crud_firmware.download_firmware_file_by_version(
                db=db,
                firmware_version=firmware_version,
                file_type=file_type,
                bucket_name=settings.GCS_BUCKET_NAME,
...
def update_firmware(
    ...
    updated_firmware = crud_firmware.update(
        db=db,
        db_obj=firmware,
