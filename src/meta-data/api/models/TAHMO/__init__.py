...
class apiWrapper(object):
    ...
    def getMeasurements(
        ...
        endpoint = "services/measurements/v2/stations/%s/measurements/%s" % (
            station,
            dataset,
        )
        ...
        for index, row in dateSplit.iterrows():
            ...
            if (
                ...
                for result in response["results"]:
                    if (
                        "series" in result
                        and len(result["series"]) >= 1
                        and "values" in result["series"][0]
                    ):
        ...
        for shortcode in seriesHolder:
            ...
            if len(timestamps) > len(set(timestamps)):
                ...
                for sensor in sensors:
                    sensorSerie = list(
                        filter(lambda x: x[2] == sensor, seriesHolder[shortcode])
                    )
                ...
                stations.append(
                    {
                        ...
                        "name": element.get("location", {}).get("name", None),
                        "timezone": element.get("location", {}).get("timezone", None),
                        "countrycode": element.get("location", {}).get(
                            "countrycode", None
                        ),
                        "latitude": element.get("location", {}).get("latitude", None),
                        "longitude": element.get("location", {}).get("longitude", None),
                        "type": element.get("location", {}).get("type", None),
                    }
                )
        return stations
    ...
    def __splitDateRange(self, inputStartDate, inputEndDate):
        ...
        df = pd.DataFrame(
            [
                [i, x]
                for i, x in zip(dates, dates.shift(1) - datetime.timedelta(seconds=1))
            ],
            columns=["start", "end"],
        )
    ...
    def __handleApiError(self, apiRequest):
        ...
        try:
            ...
            if json and "error" in json and "message" in json["error"]:
                ...
                raise Exception(
                    "API request failed with status code %s" % apiRequest.status_code
                )
