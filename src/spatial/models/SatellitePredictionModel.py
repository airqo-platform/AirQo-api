...
class SatellitePredictionModel:
    ...
    @staticmethod
    def get_satellite_data(aoi: ee.Geometry.Point, collection: str, fields: List[str]) -> ee.FeatureCollection:
        return (ee.ImageCollection(collection)
                .filterDate(datetime.now(timezone.utc) - timedelta(days=7), datetime.now(timezone.utc))
                .filterBounds(aoi)
                .select(fields)
                .map(lambda image: SatellitePredictionModel.extract_data_for_image(image, aoi)))

    @staticmethod
    def extract_data_for_location(location: Dict[str, Any], collections: Dict[str, List[str]]) -> Dict[str, Any]:
