...
class SpatialDataHandler_confidence:
    @staticmethod
    def get_air_quality_data_getis():
        try:
            post_data = request.get_json()

            if not all(
                key in post_data for key in ["grid_id", "start_time", "end_time"]
            ):
            ...
            try:
                ...
                return (
                    jsonify(
                        {
                            "error": "Invalid datetime format for start_time or end_time. Required format: YYYY-MM-DDTHH:MM:SS"
                        }
                    ),
                    400,
                )
            ...
            if not site_ids:
                return (
                    jsonify(
                        {"error": "No air quality data available for the specified id."}
                    ),
                    404,
                )
            ...
            if results is None:
                return (
                    jsonify({"error": "Error querying BigQuery for air quality data."}),
                    500,
                )
            ...
            significant_hot_spots_99_geojson = convert_dataframe_to_geojson(
                significant_hot_spots_99_df
            )
            significant_hot_spots_95_geojson = convert_dataframe_to_geojson(
                significant_hot_spots_95_df
            )
            significant_hot_spots_90_geojson = convert_dataframe_to_geojson(
                significant_hot_spots_90_df
            )
            significant_cold_spots_99_geojson = convert_dataframe_to_geojson(
                significant_cold_spots_99_df
            )
            significant_cold_spots_95_geojson = convert_dataframe_to_geojson(
                significant_cold_spots_95_df
            )
            significant_cold_spots_90_geojson = convert_dataframe_to_geojson(
                significant_cold_spots_90_df
            )
            not_significant_df_geojson = convert_dataframe_to_geojson(
                not_significant_df
            )
