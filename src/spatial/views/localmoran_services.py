...
from models.localmoran_model import AirQualitySpatilaAnalyzer_moran


class SpatialDataHandler_moran:
    @staticmethod
    def get_air_quality_data_moran():
        try:
            ...
            analyzer = AirQualitySpatilaAnalyzer_moran()
            ...
            if not site_ids:
                return (
                    jsonify(
                        {
                            "error": "No air quality data available for the specified parameters."
                        }
                    ),
                    404,
                )
            ...
            if results is None:
                ...
                return (
                    jsonify(
                        {
                            "error": "Error querying BigQuery for air quality data.",
                            "details": str(e),
                        }
                    ),
                    500,
            ...
            moran_result = analyzer.moran_local(
                analyzer.moran_local_regression(gdf), gdf
            )
            moran_result_list = moran_result.tolist()
            moran_result_num = analyzer.moran_num_local(
                analyzer.moran_local_regression(gdf), gdf
            )
            ...
            gdf_geojson = FeatureCollection(
                [
                    ...
                    for xy, val, cluster, moran_num in zip(
