...
const SearchHistoryModel = (tenant) => {
    try {
        let searchHistories = mongoose.model("searchHistories");
        return searchHistories;
    } catch (error) {
        let searchHistories = getModelByTenant(tenant, "searchHistory", SearchHistorySchema);
        return searchHistories;
    }
};
...
const searchHistories = {
    ...
    list: async (request) => {
        try {
            ...
            const responseFromListSearchHistories = await responseFromListSearchHistoriesPromise;
    ...
    syncSearchHistories: async (request) => {
        ...
        try {
            ...
            let unsynced_search_histories = (await SearchHistoryModel(
                tenant.toLowerCase()
            ).list({ filter })).data;
            ...
            const missing_search_histories = search_histories.filter((item) => {
                const found = unsynced_search_histories.some((search_history) => {
                    return search_history.place_id === item.place_id && search_history.firebase_user_id === item.firebase_user_id;
                });
                return !found;
            });
            ...
            let synchronizedSearchHistories = (await SearchHistoryModel(
                tenant.toLowerCase()
            ).list({ filter })).data;
