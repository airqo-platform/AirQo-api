// middleware/enhancedPermissionAuth.js
...
const logger = log4js.getLogger(
  `${constants.ENVIRONMENT} -- enhanced-permission-auth`
);

/**
 * Enhanced universal token middleware - handles all token strategies automatically
 */
const createUniversalTokenMiddleware = () => {
  return async (req, res, next) => {
    try {
      ...
      console.log(
        "🔐 Enhanced Token Middleware: Processing token for tenant:",
        tenant
      );

      try {
        // Use Enhanced Token Factory for universal token decoding
        const tokenFactory = new EnhancedTokenFactory(tenant);
        const decodedUser = await tokenFactory.decodeToken(token);

        console.log(
          "✅ Enhanced Token Middleware: Token decoded successfully:",
          {
            userId: decodedUser.userId || decodedUser.id || decodedUser._id,
            strategy: decodedUser._tokenStrategy,
            permissionsCount: decodedUser.permissions?.length || 0,
          }
        );
        ...
        req.user = {
          ...user.toObject(),
          // Enhanced fields from token
          permissions:
            decodedUser.permissions || decodedUser.allPermissions || [],
          systemPermissions: decodedUser.systemPermissions || [],
          groupPermissions: decodedUser.groupPermissions || {},
          networkPermissions: decodedUser.networkPermissions || {},
          groupMemberships: decodedUser.groupMemberships || [],
          networkMemberships: decodedUser.networkMemberships || [],
          isSuperAdmin: decodedUser.isSuperAdmin || false,
          _tokenStrategy: decodedUser._tokenStrategy || "unknown",
          _decodedAt: decodedUser._decodedAt,
        };

        req.tokenStrategy = decodedUser._tokenStrategy || "unknown";

        console.log(
          "✅ Enhanced Token Middleware: User authenticated successfully"
        );
        next();
      } catch (tokenError) {
        console.error(
          "❌ Enhanced Token Middleware: Token decode error:",
          tokenError.message
        );
        return res.status(401).json({
          success: false,
      ...
      console.error("❌ Enhanced Token Middleware: Unexpected error:", error);
...
// Export the universal token middleware as the main auth method
const enhancedAuth = createUniversalTokenMiddleware();

module.exports = {
  // Main authentication middleware
  enhancedAuth,
  createUniversalTokenMiddleware,
