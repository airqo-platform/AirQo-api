...
const userTypeUtil = require("@utils/user-type.util");

const isEmpty = require("is-empty");
const constants = require("@config/constants");
const log4js = require("log4js");
const logger = log4js.getLogger(
  `${constants.ENVIRONMENT} -- user-type-controller`
);
...
const createUserType = {
  listUsersWithUserType: async (req, res, next) => {
    try {
      ...
      const result = await userTypeUtil.listUsersWithUserType(request, next);
  ...
  listAvailableUsersForUserType: async (req, res, next) => {
    try {
      ...
      const result = await userTypeUtil.listAvailableUsersForUserType(
        request,
        next
      );
  ...
  assignUserType: async (req, res, next) => {
    try {
      ...
      const result = await userTypeUtil.assignUserType(request, next);
  ...
  assignManyUsersToUserType: async (req, res, next) => {
    try {
      ...
      const result = await userTypeUtil.assignManyUsersToUserType(
        request,
        next
      );
