...
const createClient = {
  create: async (req, res, next) => {
    try {
      ...
      const result = await clientUtil.createClient(request, next);
  ...
  getById: async (req, res, next) => {
    try {
      ...
      // Prepare request object with tenant handling (consistent with other methods)
      const request = req;
      const defaultTenant = constants.DEFAULT_TENANT || "airqo";
      request.query.tenant = isEmpty(req.query.tenant)
        ? defaultTenant
      ...
      // Use the dedicated util method
      const result = await clientUtil.getClientById(request, next);
      ...
      if (result.success === true) {
        ...
        return res.status(status).json({
          success: true,
          message: result.message ? result.message : "",
          clients: result.data ? result.data : null,
        });
      } else if (result.success === false) {
        const status = result.status
          ? result.status
  ...
  list: async (req, res, next) => {
    try {
      ...
      if (req.params.client_id) {
        return await createClient.getById(req, res, next);
      }
  ...
  delete: async (req, res, next) => {
    try {
      ...
      const result = await clientUtil.deleteClient(request, next);
  ...
  update: async (req, res, next) => {
    try {
      ...
      const result = await clientUtil.updateClient(request, next);
  ...
  activateClient: async (req, res, next) => {
    try {
      ...
      const result = await clientUtil.activateClient(request, next);
  ...
  activateClientRequest: async (req, res, next) => {
    try {
      ...
      const result = await clientUtil.activateClientRequest(request, next);
  ...
  updateClientSecret: async (req, res, next) => {
    try {
      ...
      const result = await clientUtil.updateClientSecret(request, next);
