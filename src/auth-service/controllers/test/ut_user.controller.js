...
describe("createUserController", () => {
  ...
  describe("generateVerificationToken", () => {
    it("should generate and return the verification token", async () => {
      ...
      // Mock the tokenUtil.generateVerificationToken function to return success
      sinon.stub(tokenUtil, "generateVerificationToken").resolves({
        success: true,
      ...
      // Restore the stubs
      validationResult.isEmpty.restore();
      tokenUtil.generateVerificationToken.restore();
    });
    ...
    it("should return an internal server error response on error", async () => {
      ...
      // Mock the tokenUtil.generateVerificationToken function to throw an error
      sinon
        .stub(tokenUtil, "generateVerificationToken")
  ...
  describe("verifyVerificationToken", () => {
    it("should verify the verification token and return a successful response", async () => {
      ...
      // Mock the tokenUtil.verifyVerificationToken function to return success
      sinon.stub(tokenUtil, "verifyVerificationToken").resolves({
        success: true,
        status: httpStatus.OK,
      ...
      // Restore the stubs
      validationResult.isEmpty.restore();
      tokenUtil.verifyVerificationToken.restore();
    });
    ...
    it("should return an internal server error response on error", async () => {
      ...
      // Mock the tokenUtil.verifyVerificationToken function to throw an error
      sinon
        .stub(tokenUtil, "verifyVerificationToken")
        .throws(new Error("example-error"));
  ...
  describe("verifyEmail", () => {
    it("should return a successful response with email verified message when email is verified", async () => {
      ...
      // Mock the tokenUtil.verifyEmail function to return a success response
    ...
    it("should return an error response when verifyEmail function returns an error response", async () => {
      ...
      // Mock the tokenUtil.verifyEmail function to return an error response
      sinon.stub(tokenUtil, "verifyEmail").resolves({
        success: false,
...
describe("googleCallback — custom-scheme redirect_after validation", () => {
  let userController;
  let req, res, next;
  let userModelStub, userUtilStub;
  ...
  before(() => {
    ...
    const fakeUser = {
      ...
      toAuthJSON: sinon.stub().resolves({ token: fakeToken }),
      findByIdAndUpdate: sinon.stub().resolves(),
    };
  ...
  it("redirects to vertex:// deep-link URL when redirect cookie is vertex://login", async () => {
  ...
  it("redirect URL contains the token hash when redirect_after is vertex://login", async () => {
    req.cookies._oauth_redirect_after = "vertex://login";
    await userController.googleCallback(req, res, next);
    expect(res.redirect.firstCall.args[0]).to.include("#token=");
  });

  it("redirects to deep-link for case-variant VERTEX://login", async () => {
    req.cookies._oauth_redirect_after = "VERTEX://login";
    await userController.googleCallback(req, res, next);
    expect(res.redirect.firstCall.args[0]).to.match(/^VERTEX:\/\//i);
  });

  it("redirects to beacon:// deep-link when redirect cookie is beacon://home", async () => {
    req.cookies._oauth_redirect_after = "beacon://home";
    await userController.googleCallback(req, res, next);
    expect(res.redirect.firstCall.args[0]).to.match(/^beacon:\/\//);
  });

  it("falls back to the default analytics URL when redirect_after is an unrecognised scheme", async () => {
  ...
  it("falls back to the default analytics URL when no redirect cookie is present", async () => {
    await userController.googleCallback(req, res, next);
    expect(res.redirect.firstCall.args[0]).to.include("analytics.airqo.net");
  });
});
