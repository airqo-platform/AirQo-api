...
ApplicationEmailConfigurationSchema.statics = {
  // Each tenant can hold multiple configurations (e.g. different application
  // email groups routing to different admin CC lists), so register() always
  // inserts a new document rather than merging into an existing one. Editing
  // an existing config is handled separately by modify().
  async register(args, next) {
    try {
      const { applicationEmails, adminCCEmails } = args;

      const doc = {
        adminCCEmails: adminCCEmails || "",
      };
      if (Array.isArray(applicationEmails) && applicationEmails.length > 0) {
        doc.applicationEmails = applicationEmails;
      }
