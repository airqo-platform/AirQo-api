...
class MessageBrokerUtils:
    ...
    def __init__(self):
        ...
        self.partition_loads = {
            p: 0 for p in self.__partitions
    ...
    def _generate_chunks(self, dataframe_list: list) -> Generator[Any]:
        """
        ...
        if column_key:
            ...
            for row in dataframe_list:
                ...
                try:
                    ...
                    logger.info(
                        f"Published message to topic {topic}, partition {partition}, key {key}"
                    )
            ...
            for chunk_data in self._generate_chunks(dataframe_list):
                ...
                # Use load-based partitioning
                partition = self.__get_least_loaded_partition()
                try:
                    producer.send(
                        topic=topic, value=message, partition=partition
                    ).add_callback(self.__on_success).add_errback(self.__on_error)
                    # Update the load count for the selected partition
                    self.partition_loads[partition] += 1
                    logger.info(
                        f"Published message to topic {topic}, partition {partition}"
                    )
                except Exception as e:
                    logger.exception(
    ...
    def consume_from_topic(
        ...
        # If a specific offset is given, seek to that offset
        ...
        try:
            for message in consumer:
                key = message.key  # Deserialized key, if enabled
                value = message.value  # Deserialized message content
