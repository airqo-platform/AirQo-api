...
def test_site_hourly_forecast_query_is_registered():
    ...
    assert "t1.site_latitude" not in hourly_query.sql
    assert "t1.site_longitude" not in hourly_query.sql
    ...
    assert "NULLIF(TRIM(t1.site_name), '')" not in daily_query.sql
    assert (
    ...
    assert "t1.site_latitude" not in daily_query.sql
    assert "t1.site_longitude" not in daily_query.sql
