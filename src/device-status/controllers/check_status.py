...
import logging
from config import db_connection, config, constants
...
@device_status_bp.route(api.route['status'], methods=['POST'])
def get_device_channel_status():
    # api_url = '{0}{1}'.format(config.BASE_API_URL, 'channels')
    tenant = request.args.get('tenant')
    if not tenant:
        return jsonify({"message": "please specify the organization name. Refer to the API documentation for details.", "success": False}), 400
    api_url = '{0}'.format(config.configuration.BASE_API_URL, 'channels')
    print(api_url)
    ...
    for channel in results:
        ...
        latest_device_status_request_api_url = '{0}{1}{2}'.format(
            config.configuration.BASE_API_URL, 'feeds/recent/', channel['channelID'])
