...
class ForecastUtils:
    @staticmethod
    def preprocess_training_data(hourly_df, daily_df):  # combined code
    ...
    @staticmethod
    def feature_eng_training_data(hourly_df, daily_df, target_column):

        def get_lag_features(df_tmp, TARGET_COL, shifts, functions):
            df_tmp = df_tmp.sort_values(by=['device_number', 'created_at'])
            for s in shifts[:2]:
                df_tmp[f'pm2_5_last_{s}_{unit}'] = df_tmp.groupby(['device_number'])[TARGET_COL].shift(s)
            for s in shifts[2:]:
                for f in functions:
                    df_tmp[f'pm2_5_{f}_{s}_{unit}'] = df_tmp.groupby(['device_number'])[TARGET_COL].shift(1).rolling(
                        s).agg(
                        f)
            return df_tmp

        def get_other_features(df_tmp, attributes):
            for a in attributes:
        ...
        global unit  # to use in get_lag_features
        unit = 'hour'
        ...
        hourly_df = get_other_features(hourly_df, hourly_attributes)
        hourly_df = get_lag_features(hourly_df, target_column, hourly_shifts, hourly_functions)

        # preprocess daily data
        unit = 'day'
        daily_df = get_lag_features(daily_df, target_column, daily_shifts, daily_functions)
        daily_df = get_other_features(daily_df, daily_attributes)

        return hourly_df, daily_df

    @staticmethod
    def train_models(hourly_data, daily_data):
        """
        Perform the actual training - models for hourly and daily data
        """
        models = {}
        for frequency, data in zip(['hourly', 'daily'], [hourly_data, daily_data]):
            print(f'Training model for {frequency} data...')
            print('feature selection started.....')
            # sort values by both device_number and created_at
            data = data.sort_values(by=['device_number', 'created_at'])
            features = [c for c in data.columns if c not in ["created_at", "pm2_5"]]
            print(features)
            target_col = "pm2_5"
            train_data, test_data = pd.DataFrame(), pd.DataFrame()

            for device_number in data['device_number'].unique():
                device_df = data[data['device_number'] == device_number]
            ...
            with mlflow.start_run():
                ...
                mlflow.sklearn.log_model(
                    sk_model=clf,
                    artifact_path=f"{frequency}_forecast_model",
                    registered_model_name=f"LGBM_{frequency}_forecast_model_dev"
                )
                ...
                clf.fit(data[features], data[target_col])

            models[frequency] = clf

        return models['hourly'], models['daily']

    @staticmethod
    def upload_trained_model_to_gcs(trained_model_hourly, trained_model_daily, project_name, bucket_name,
                                    source_blob_name_hourly, source_blob_name_daily):
        fs = gcsfs.GCSFileSystem(project=project_name)

        trained_models = {'hourly': trained_model_hourly, 'daily': trained_model_daily}
        source_blob_names = {'hourly': source_blob_name_hourly, 'daily': source_blob_name_daily}
