...
def test_fetch_hourly_forecast_training_data(mock_bigquery_client):
    api = BigQueryApi()
    api.client = mock_bigquery_client
...
def test_fetch_hourly_forecast_training_data_exception():
    api = BigQueryApi()
    api.client = mock_bigquery_client()
...
def test_fetch_daily_forecast_training_data_date_range():
    api = BigQueryApi()
    api.client = mock_bigquery_client()
    api.client.query.return_value.result.return_value.to_dataframe.return_value = (
        pd.DataFrame(
