...
def query_prediction_data():
  '''
  This is not being utilized currently.
  '''
...
def get_all_static_channels():
  '''
  Get all channels - tracked using a table from Big Query
  This also not being used
  '''
  client = bigquery.Client()
...
def preprocess_forecast_data(forecast_data_path,metadata_path, boundary_layer_path):
  ...
  # use_channels ===== get all active device on the network
...
def initialise_training_constants():
  ...
  FORECAST_DATA_PATH = 'Zindi_PM2_5_forecast_datay.csv' # events
  METADATA_PATH = 'meta.csv' # site details
  BOUNDARY_LAYER_PATH = 'boundary_layer.csv' # device_registry = boundaries collection
