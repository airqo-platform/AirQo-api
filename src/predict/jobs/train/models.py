...
class Site(BaseModel):
    ...
    def get_sites():
        ...
        try:
            api_url = f"{configuration.AIRQO_API_BASE_URL}devices/sites?tenant={configuration.TENANT}&network={configuration.TENANT}"
            print(api_url)
            authorisation_token = f"'TOK:<{configuration.AIRQO_API_AUTH_TOKEN}>'"
            results = requests.get(
                api_url, verify=False, headers={"Authorization": authorisation_token}
            )
            print(results)
    ...
    def get_events_db(self):
        ...
        projection = {
            "$project": {
                ...
                "values": {"pm2_5": 1, "device_number": 1, "time": 1},
            }
        }
...
class Devices(BaseModel):
    ...
    def get_devices():
        ...
        try:
            api_url = f"{configuration.AIRQO_API_BASE_URL}devices?tenant={configuration.TENANT}&active=yes&network={configuration.TENANT}"
            authorisation_token = f"'TOK:<{configuration.AIRQO_API_AUTH_TOKEN}>'"
            results = requests.get(
                api_url, verify=False, headers={"Authorization": authorisation_token}
            )
