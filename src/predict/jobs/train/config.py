...
class Config:
    ...
    TRAIN_DATE_HOUR_END = pd.to_datetime(
        os.environ.get(
            "TRAIN_END_DATE", datetime.strftime(datetime.now(), "%Y-%m-%d %H:00:00")
        )
    )
...
class ProductionConfig(Config):
    ...
    AIRQO_API_AUTH_TOKEN = os.getenv("AIRQO_API_AUTH_TOKEN_PROD")
...
class TestingConfig(Config):
    ...
    AIRQO_API_BASE_URL = os.getenv("AIRQO_API_BASE_URL_PROD")
    AIRQO_API_AUTH_TOKEN = os.getenv("AIRQO_API_AUTH_TOKEN_STAGE")
