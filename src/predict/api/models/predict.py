...
from models import datamanagement, processing
    ...
    if all_channel_data.empty:
        ...
        if not current_best_config:
            ...
            if len(data_to_use) == 0:
                results = {'predictions': 0, 'errors': 'no predictions, check status of the device at this location',
                           'channel_id': selected_channel_id}
                channels_with_out_predictions.append(results)
            else:
                # generating mean, lower ci, upper ci
                yhat_24, lower_ci, upper_ci = simple_forecast_ci(data_to_use, best_number_of_days_back,
                                                                 considered_hours)
                ...
                for i in range(0, len(yhat_24)):
                    ...
                    prediction_datetime = pd.to_datetime(
                        prediction_start_datetime + datetime.timedelta(hours=hour_to_add))
                    ...
                    model_predictions_tuple = (
                        model_name, channel_id, location_name, location_latitude, location_longitude,
                        prediction_value, prediction_start_datetime, prediction_datetime,
...
def sarima_forecast(history, config):
    ...
    # define model
    model = SARIMAX(series, order=order, seasonal_order=sorder, trend=trend, enforce_stationarity=False,
