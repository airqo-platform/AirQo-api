...
@api.route(routes.route["next_24hr_forecasts"])
class GetNext24hrForecasts(Resource):
...
@api.route(routes.route["next_1_week_forecasts"])
class GetNextWeekForecasts(Resource):
...
@api.route(routes.route["all_1_week_forecasts"])
class GetAllDailyForecasts(Resource):
...
@api.route(routes.route["all_24hr_forecasts"])
class GetAllHourlyForecasts(Resource):
...
@api.route(routes.route["predict_for_heatmap"])
class PredictionsForHeatmap(Resource):
...
@api.route(routes.route["search_predictions"])
class SearchPredictions(Resource):
...
@api.route(routes.route["parish_predictions"])
class ParishPredictions(Resource):
