...
class Validator(OrderedDict):
    ...
    def __init__(self, validation_type=""):
        ...
        self["list"] = self.type_validator(
            self.list_checker, "{} is not a valid list/array"
        )
        self["bool"] = self.type_validator(
            self.str_to_bool, "{} is not a valid boolean"
        )
        self["dict"] = self.type_validator(dict, "{} is not a valid dict")
        self["str"] = self.type_validator(str, "{} is not a valid string")
        self["date"] = self.type_validator(
            self.date_checker, "{} is not a valid date. Date format is '%Y-%M-%D'"
        )
        ...
        self["email"] = self.type_validator(
            self.email_checker, "{} is not a valid email"
        )
