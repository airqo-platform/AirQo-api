...
class PreRequest:
    ...
    TENANT_KEY = "tenant"
    DOCS_ENDPOINTS = [
        f"{API_V2_BASE_URL}/apidocs",
        f"{API_V2_BASE_URL}/flasgger",
        f"{API_V2_BASE_URL}/apispec",
    ]
    IGNORE_TENANT_HEADER = ["/health"]
    COMBINED_IGNORE_TENANT_HEADER = (
        f"({')|('.join(chain(DOCS_ENDPOINTS, IGNORE_TENANT_HEADER))})"
    )
